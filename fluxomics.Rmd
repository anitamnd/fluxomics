...
groups <- groups[!groups %in% c("Blank", "QC no label")]
...
selectedIso <- reactive({
  ...
  data <- normalizedZero %>%
    ...
    filter(names %in% input$isotopologues) %>%
    select(-where(~all(is.na(.)))) %>%
    pivot_longer(names_to = "names", values_to = "x") %>%
...
renderPlot({
  ggplot(selectedIso(), aes(fill = group_time, x = names, y = x)) +
        geom_bar(position = "dodge", stat = "identity") +
...
Compare within a group (changes over time)